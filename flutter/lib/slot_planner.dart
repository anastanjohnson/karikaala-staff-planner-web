...
class _StaffWeeklyAgenda extends StatefulWidget {
  const _StaffWeeklyAgenda({required this.name, required this.slots});
  final String name;
  final List<WorkSlot> slots;
  @override
  State<_StaffWeeklyAgenda> createState() => _StaffWeeklyAgendaState();
}

class _StaffWeeklyAgendaState extends State<_StaffWeeklyAgenda> {
  static const green = Color(0xFF24573D);
  static const softGreen = Color(0xFFE8F1EB);
  ...
  bool closed(DateTime date) =>
      date.weekday == DateTime.tuesday || date.weekday == DateTime.wednesday;
  void changeWeek(DateTime week, int direction) {
    final next = week.add(Duration(days: direction * 7));
    final end = next.add(const Duration(days: 7));
    final inWeek = widget.slots.where(
      (s) => !s.date.isBefore(next) && s.date.isBefore(end),
    );
    setState(() {
      shownWeek = next;
      selected = inWeek.isEmpty ? next : inWeek.first.date;
    });
  }
  ...
  Widget build(BuildContext context) {
    final today = dateOnly(DateTime.now());
    final defaultDay = widget.slots.isEmpty ? today : widget.slots.first.date;
    final week = shownWeek ?? monday(defaultDay);
    final end = week.add(const Duration(days: 7));
    final chosen = selected ?? defaultDay;
    final weekSlots = widget.slots
        .where((s) => !s.date.isBefore(week) && s.date.isBefore(end))
        .toList();
    final chosenSlots = weekSlots
        .where((s) => dateKey(s.date) == dateKey(chosen))
        .toList();
    final remaining = <String, List<WorkSlot>>{};
    for (final slot in weekSlots) {
      if (dateKey(slot.date) != dateKey(chosen)) {
        remaining.putIfAbsent(dateKey(slot.date), () => []).add(slot);
      }
    }
    final isThisWeek = dateKey(week) == dateKey(monday(today));
    final isNext =
        chosenSlots.isNotEmpty &&
        widget.slots.isNotEmpty &&
        chosenSlots.first.id == widget.slots.first.id;
    final monthLabel = week.month == end.subtract(const Duration(days: 1)).month
        ? '${months[week.month - 1]} ${week.year}'
        : '${shortDate(week)} – ${shortDate(end.subtract(const Duration(days: 1)))} ${end.subtract(const Duration(days: 1)).year}';
    return ListView(
      ...
      children: [
        Row(
          children: [
            Expanded(
              child: Text(
                isThisWeek ? 'This week' : 'Your week',
                style: const TextStyle(
                  fontSize: 22,
            ...
            if (!isThisWeek)
              TextButton(
                onPressed: () {
                  final thisWeek = monday(today);
                  final nextWeek = thisWeek.add(const Duration(days: 7));
                  final shifts = widget.slots.where(
                    (s) =>
                        !s.date.isBefore(thisWeek) && s.date.isBefore(nextWeek),
                  );
                  setState(() {
                    shownWeek = thisWeek;
                    selected = shifts.isEmpty ? thisWeek : shifts.first.date;
                  });
                },
        ...
        Row(
          children: [
            Expanded(
              child: Text(monthLabel, style: const TextStyle(fontSize: 13)),
            ),
            IconButton(
              tooltip: 'Previous week',
              onPressed: () => changeWeek(week, -1),
              icon: const Icon(Icons.chevron_left),
            ),
            IconButton(
              tooltip: 'Next week',
                ...
                Expanded(
                  child: Builder(
                    builder: (context) {
                      ...
                      final isClosed = closed(day);
                      final active =
                          !isClosed && dateKey(day) == dateKey(chosen);
                      final hasShift = weekSlots.any(
                        (s) => dateKey(s.date) == dateKey(day),
                      );
                      final shiftDay = !isClosed && hasShift;
                      return Semantics(
                        label:
                            '${dayNames[i]}, ${shortDate(day)}${isClosed ? ', restaurant closed' : (hasShift ? ', assigned shift' : ', no shift')}',
                        selected: active,
                        child: InkWell(
                          onTap: isClosed
                              ? null
                          ...
                          child: Container(
                            ...
                            decoration: BoxDecoration(
                              color: shiftDay
                                  ? green
                                  : (isClosed ? rosterSurface : Colors.white),
                              border: Border.all(
                                width: 2,
                                color: active
                                    ? (shiftDay ? Colors.white : green)
                                    : Colors.transparent,
                              ),
                            ),
                            child: Column(
                              children: [
                                ...
                                Text(
                                  ...
                                  style: TextStyle(
                                    ...
                                    color: shiftDay
                                        ? Colors.white
                                        : (isClosed ? rosterMuted : rosterInk),
                                  ),
                                ),
                                const SizedBox(height: 3),
                                SizedBox(
            ...
            children: [
              Row(
                ...
                children: [
                  Expanded(
                    child: Text(
                      '${dayNames[chosen.weekday - 1]}, ${chosen.day} ${months[chosen.month - 1]}',
                      style: const TextStyle(
                        color: green,
                        fontWeight: FontWeight.w700,
                        fontSize: 14,
              ...
              if (chosenSlots.isEmpty)
                Text(
                  closed(chosen)
                      ? 'Restaurant closed'
                      : 'No upcoming shifts on this day.',
                  style: const TextStyle(color: rosterMuted, fontSize: 14),
                ),
                  ...
                  children: [
                    Expanded(
                      child: FittedBox(
                        ...
                        child: Text(
                          '${clock(chosenSlots[i].start)}–${clock(chosenSlots[i].end)}',
                          style: const TextStyle(
                            fontSize: 24,
                            height: 1.2,
                            fontWeight: FontWeight.w700,
                    ...
                    Text(
                      _StaffShiftDateCard(
                        slots: chosenSlots,
                        isNext: false,
                      ).hoursLabel(chosenSlots[i]),
                      style: const TextStyle(fontSize: 12, color: rosterMuted),
                    ),
                  ],
                ),
                if (chosenSlots[i].nextDay)
                  const Padding(
                    padding: EdgeInsets.only(top: 3),
                    child: Text(
                      'Ends the next day',
                      style: TextStyle(fontSize: 11, color: rosterMuted),
                    ),
                  ),
                if (chosenSlots[i].breakMinutes > 0)
                  Padding(
                    padding: const EdgeInsets.only(top: 3),
                    child: Text(
                      '${chosenSlots[i].breakMinutes} min unpaid break',
        ...
        if (remaining.isNotEmpty) ...[
          const SizedBox(height: 10),
          Text(
            isNext ? 'Coming up' : 'Other shifts this week',
          ...
          for (final entry in remaining.entries)
            _StaffShiftDateCard(
              ...
              isNext: false,
              compact: true,
              onTap: () => setState(() => selected = entry.value.first.date),
            ),
        ],
        if (weekSlots.isEmpty)
          const Padding(
            ...
            child: Text(
              'No upcoming shifts this week. Use the arrows to browse other weeks.',
              style: TextStyle(color: rosterMuted),
            ),
          ),
        const Padding(
          ...
          child: Text(
            'Your manager may update these shifts.',
            style: TextStyle(fontSize: 12, color: rosterMuted),
          ),
        ),
      ],
...
class _StaffShiftDateCard extends StatelessWidget {
  const _StaffShiftDateCard({
    required this.slots,
  ...
  String hoursLabel(WorkSlot slot) {
    final minutes = slot.duration - slot.breakMinutes;
  ...
  Widget compactCard(BuildContext context) {
    ...
    return Semantics(
      container: true,
      label: '${dayNames[date.weekday - 1]}, ${shortDate(date)} ${date.year}',
      child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        for (var i = 0; i < slots.length; i++) ...[
                            ...
                            children: [
                              Expanded(
                                child: FittedBox(
                                  ...
                                  alignment: Alignment.centerLeft,
                                  child: Text(
                                    '${clock(slots[i].start)}–${clock(slots[i].end)}',
                                    style: const TextStyle(
                                      fontSize: 18,
                                      height: 1.2,
                                      fontWeight: FontWeight.w700,
                          ...
                          if (slots[i].breakMinutes > 0)
                            Padding(
                              padding: const EdgeInsets.only(top: 3),
                              child: Text(
                                '${slots[i].breakMinutes} min unpaid break',
                                style: const TextStyle(
                                  fontSize: 11,
                                  color: rosterMuted,
                                ),
  ...
  @override
  Widget build(BuildContext context) {
    if (compact) return compactCard(context);
    final date = slots.first.date;
    final foreground = isNext ? Colors.white : rosterInk;
    final secondary = isNext ? const Color(0xFFD8E8DF) : rosterMuted;
    return Semantics(
      container: true,
      label: '${dayNames[date.weekday - 1]}, ${shortDate(date)} ${date.year}',
      child: GestureDetector(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            border: Border.all(color: isNext ? green : rosterLine),
          ),
          child: Row(
            ...
            children: [
              Container(
                ...
                decoration: BoxDecoration(
                  color: isNext ? Colors.white.withValues(alpha: 0.10) : green,
                ...
                child: Column(
                  ...
                  children: [
                    Text(
                      dayNames[date.weekday - 1].substring(0, 3).toUpperCase(),
                      style: const TextStyle(
                        color: Colors.white,
                        fontSize: 13,
                        fontWeight: FontWeight.w700,
                    ...
                    for (var i = 0; i < slots.length; i++) ...[
                      ...
                      FittedBox(
                        ...
                        child: Text(
                          ...
                          style: TextStyle(
                            color: foreground,
                            fontSize: compact ? 20 : (isNext ? 28 : 24),
                            fontWeight: FontWeight.w700,
                            height: 1.2,
                          ),
                        ),
              ...
              if (onTap != null) ...[
                const SizedBox(width: 8),
                const Icon(Icons.chevron_right, size: 20, color: rosterMuted),
              ],
            ],
          ),
        ),
